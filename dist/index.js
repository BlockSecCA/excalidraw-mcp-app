...
try {
  ...
  DIST_DIR = __filename.endsWith(".ts") ? path.join(__dirname, "dist") : __dirname;
} catch (err) {
  console.error("[Excalidraw] Failed to resolve paths:", err);
  process.exit(1);
}
var checkpointStore = /* @__PURE__ */ new Map();
function extractCheckpointRestore(elements) {
  let restoreId = null;
  let viewport = null;
...
Call read_me first to learn the element format.`,
    ...
    async ({ elements }) => {
      ...
      let restoredViewport;
      if (restoreId) {
        const stored = checkpointStore.get(restoreId);
        if (stored) {
          restoredElements = deleteIds.size > 0 ? stored.elements.filter((el) => !deleteIds.has(el.id)) : stored.elements;
          restoredViewport = stored.viewport;
        }
      }
      const checkpointId = Math.random().toString(36).slice(2, 8);
      const allElements = restoredElements ? [...restoredElements, ...newElements] : newElements;
      checkpointStore.set(checkpointId, {
        elements: allElements,
        viewport: viewport ?? restoredViewport
      });
      const structuredContent = { checkpointId };
      if (restoredElements) {
